...
I recently implemented a newsletter subscription feature on [fitvitfitness.com](https://www.fitvitfitness.com). Site needs to accumulates future client's emails using a 3rd party service 
called MailChimp. MailChimp, for those of you don't know, it's a service that gather emails and capable of sending newsletters directly to your subscriber's email inbox. 
...
Obiviously the two main choices comes to mind for every experienced developer is microservice or a one big code base using monolithic.

A microservice is an independent services that communicates over well-defined APIs. Microservices can be focused for sepefic modules or features for example:

- handling image uploads and resizing.
- Rest apis.
- Cron job tasks

A Monolithic architecture, all features or modules are tightly coupled and run as a single service. Basically having one code base that has all the features.
