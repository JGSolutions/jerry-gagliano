...
For developers, I recommend learning HTML, CSS and JavaScript first! Don't jump in the bag wagon with ReactJS and build simple
websites as single page app.
...
you might run into surprises that can costly heavily in the long run.
...
websites from the server which is (MPSa) or use a static server generator framework as I will explain below.

<div id="advantages">
    ## Advantages of SPAs
</div>

With SPAs, all static files (CSS, Javascript) are loaded on the inital load. Users will instantly render views with data
as they navigate through the app. It is the perfect choice for creating apps with a rich user interface like mobile native apps
and delightful features.
...
No need for a backend server to render your pages like traditional websites like PHP or Python. Since your SPA is a separate
layer and you can request your data through another microservice which will require less execution and more speed.

Here is an article I wrote about  [Choosing Mircoservice over Monolithic Architecture](https://jgsolutions.ca/posts/microservices-architecture-over-monolithic/)
...
Usually, SPAs initial load begins with a specific route. It may be a /login, /dashboard, /registration etc. The initial Javascript code that will
execute the inital app. Once that is loaded, only then the Javascript for that specific view is rendered. All your other views will not be loaded on the client
side until they navigate to that view! Frameworks like Angular CLI, Vue.js and React Routing offer this pattern. Using Angular, I was able to reach scores
performance over 90 and above.
